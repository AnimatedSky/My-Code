...
with open ('D:\GitHub\COSMIC\zooniverse\classifications\cosmic-classifications 04-19-2018.csv', 'r') as FreshImpacts:
...
for num in range(len(subjects)):
	if len(subjects)  > 3: #removes cats
		continue
	else:
...
#------PULLS REST OF DATA-----
labeled_impacts = []
for u,t,a,s in zip(user,time, annotations, subjects):#RE-ZIP DATA TO MORE LEGIBLE ADN ASSIGN DATA TO GROUP
	assert (len(a)==1)
	annotation = a[0]
...
#----------TEST-------------
print(labeled_impacts[0].metadata.user)